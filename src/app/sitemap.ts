...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [tools, categories, industries] = await Promise.all([
    prisma.tool.findMany({ where: { isActive: true }, select: { slug: true, updatedAt: true } }),
  ...
  return [...staticPages, ...toolPages, ...categoryPages, ...industryPages];
}
